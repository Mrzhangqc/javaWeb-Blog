...
public class UserDao {
	public List<UserSingle> getTopList() throws SQLException{
		String sql="select top 10 id,user_name,user_hitNum from tb_user order by user_hitNum desc";
		List<UserSingle> topList=null;
		
		DB mydb=new DB();
		mydb.doPstm(sql,null);
		ResultSet rs=mydb.getRs();
		if(rs!=null){
			topList=new ArrayList<UserSingle>();
			while(rs.next()){
	...
	public List<UserSingle> getMasterList() throws SQLException{
		String sql="select * from tb_user order by user_blogName";		
		List<UserSingle> masterlist=getList(sql,null);
		return masterlist;
	}
	...
	private List<UserSingle> getList(String sql,Object[] params) throws SQLException{
		List<UserSingle> list=null;
		DB mydb=new DB();
		mydb.doPstm(sql,params);
		
		ResultSet rs=mydb.getRs();
		if(rs!=null){
			list=new ArrayList<UserSingle>();
			while(rs.next()){
				UserSingle single=new UserSingle();
				single.setId(rs.getInt(1));
				single.setUserName(rs.getString(2));
